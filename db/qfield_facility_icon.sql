...
COMMENT ON TABLE  qfield.facility_icon            IS '시설물 지도 아이콘(종류별 글리프·색상·판별 키워드)';
COMMENT ON COLUMN qfield.facility_icon.icon_type  IS '아이콘 식별자 (parking, charger, hall, dining, sports, exhibition, default)';
COMMENT ON COLUMN qfield.facility_icon.keywords   IS 'fclt_nm 부분일치 판별 키워드 배열';
COMMENT ON COLUMN qfield.facility_icon.glyph_svg  IS '핀 안쪽 SVG 조각(24x32 viewBox 기준). 문자열 COLOR 가 핀 색으로 치환됨';
COMMENT ON COLUMN qfield.facility_icon.is_default IS '종류 미판별 시 사용하는 기본 아이콘 여부(한 건만 true)';

-- 기본 아이콘은 한 건만 존재하도록 보장
CREATE UNIQUE INDEX IF NOT EXISTS ux_facility_icon_default
    ON qfield.facility_icon ((is_default)) WHERE is_default;

-- ----------------------------------------------------------------------------
-- 초기 데이터 (프론트엔드에 하드코딩되어 있던 값과 동일)
-- 다시 실행해도 안전하도록 icon_type 기준 UPSERT
-- ----------------------------------------------------------------------------
INSERT INTO qfield.facility_icon
...
-- GRANT SELECT ON qfield.facility_icon TO mcp_readonly;

-- 확인용 조회
-- SELECT icon_type, icon_label, keywords, sort_order, is_default, use_yn FROM qfield.facility_icon ORDER BY sort_order;
